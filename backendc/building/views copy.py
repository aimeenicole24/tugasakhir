...
class BuildingInputView(View):  
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            return render(request, 'building/add_building.html', {
                ...
                'distance': distance,
                'protection_level': protection_level,  # Display the calculated protection level
                'risk_index': risk_index,  # Mengirimkan nilai risk index ke template
                'level_of_damage': level_of_damage,  # Send Level of Damage to template
                'protection_recommendations': protection_recommendations,  # Send protection recommendations to template
                'arrester_data': arrester_data  # Send arrester installation data
            })
        else:
            return render(request, 'building/add_building.html', {'form': form})
    ...
    def calculate_number_of_rods(self, length, width, height):
        ...
        return max(1, int(area / 500))  # 1 lightning rod per 500 m²

    def calculate_rod_placement(self, protection_radius, mesh_distance):
        """Calculate lightning rod placement based on protection radius and mesh distance."""
...
class DashboardDataView(APIView):
    ...
    def get(self, request):
        ...
        # Ambil proyek pertama yang aktif (untuk menampilkan informasi proyek aktif)
        active_project = projects.filter(status='active').first()
