...
const RollingSphere = ({ x, z, radius, height }) => {
  ...
  return (
    <mesh ref={ref} position={[x, height + radius, z]} renderOrder={3}>
...
const HeatmapCell = ({ x, z, coverage, yOffset }) => {
  const colors = ['red', 'yellow', 'green'];
  return (
    <mesh position={[x, yOffset, z]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={4}>
...
const GroundingElectrode = ({ x, z, soilType, radius }) => {
  ...
  return (
    <mesh position={[x, -groundingLength / 2, z]} renderOrder={2}>
  ...
  return (
    <group>
      ...
      {rodPositions.map(([x, z], i) => (
        <group key={i}>
          <mesh position={[x, main_height + 25, z]} renderOrder={2}>
            <cylinderGeometry args={[0.5, 0.5, 15]} />
            <meshStandardMaterial color="red" />
          </mesh>
          <mesh position={[x, main_height / 2 + 12.5, z]} renderOrder={2}>
            <cylinderGeometry args={[0.3, 0.3, main_height + 25]} />
      ...
      {spherePositions.map(([x, z], i) => (
        <RollingSphere key={`sphere-${i}`} x={x} z={z} radius={protection_radius} height={main_height} />
      ))}

      {heatmap.map(({ x, z, coveredCount }, i) => (
        <HeatmapCell key={`heatmap-${i}`} x={x} z={z} coverage={coveredCount} yOffset={main_height + 20} />
      ))}
    </group>
  );
};
